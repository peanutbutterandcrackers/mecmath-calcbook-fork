\chapter{Applications of Integrals}\label{ch:appl-of-integrals}
%Begin Section 8.1
\section{Area Between Curves}
The ``area under a curve'' was defined in Chapter~\ref{ch:integral} as the area below some curve
...
&=~ \frac{1}{2}\,x^2 - \frac{1}{3}\,x^3~\Biggr|_0^1 ~=~ 
...
\parpic[r]{\begin{tikzpicture}[>=latex,every node/.style={font=\small},scale=0.75]
 ...
 \draw[->,black!60,line width=1pt] (0,-1.3) -- (0,2.5) node[above] {$y$};
...
\parpic[r]{\begin{tikzpicture}[>=latex,every node/.style={font=\small}]
 ...
 \draw[linecolor,line width=1.5pt,domain=pi/6:pi/3,samples=500,smooth]
   plot (xy polar cs:angle=\x r, radius={2 + 2*cos(\x r)});
 \node[above right] at (60:3) {$r_1=1+\cos \theta$};
 \node[below left] at (0,0) {$O$};
 \node[right] at (25:0.4) {$r_2=1-\cos \theta$};
\end{tikzpicture}}
...
\newpage
\[
\avg{f} ~\approx~ \sum_{i=1}^n \frac{f(x_i)}{n} ~=~ \sum_{i=1}^n \frac{f(x_i)}{n}\,\cdot\,\frac{b-a}{b-a} ~=~
\frac{1}{b-a}\,\sum_{i=1}^n f(x_i)\,\cdot\,\frac{b-a}{n} ~=~ \frac{1}{b-a}\,\sum_{i=1}^n f(x_i)\,\Delta x_i
\]
...
$y^2 = 9\left(1 - \frac{x^2}{25}\right) = \frac{9}{25}\left(25-x^2\right)$. So by the distance
...
&=~ (x-4)^2 ~+~ \frac{9}{25}\left(25-x^2\right)\\[6pt]
...
d ~&=~ \pm\,\frac{4x - 25}{5} ~=~ -\frac{4x - 25}{5} ~=~ \frac{25 - 4x}{5} ~\text{, for $-5 \le x \le 5$}\quad
\left(~
\begin{minipage}[c]{0.33\linewidth}
  \small
  since $d = (4x-25)/5 < 0$ on $\ival{-5}{5}$ and
  distance cannot be negative.
\end{minipage}
~\right)
\end{align*}

\noindent Note that by symmetry of the ellipse about the
...
\newpage
%Begin Section 8.3
\section{Arc Length and Curvature}\label{sec:arc-length-and-curvature}
Just like the area of a plane region can be found using calculus, so too can the
length of a plane curve. Along the way the mystery mentioned in \CRef{ftnt:non-pythagorean}{a footnote} in
Chapter~\ref{ch:derivative} will finally be solved: what is the length of the hypotenuse of a
...
\quad\left(\text{since $e^2 = \frac{c^2}{a^2} = \frac{a^2 - b^2}{a^2}$}\right)
\end{align*}
...
\end{tikzpicture}}
In Chapter~\ref{ch:appl-of-derivs} you saw one simple measure of curvature: the second derivative.
...
curve. Let $\alpha$ be the angle between the tangent line to the curve at $A$
and the tangent line to the curve at $B$, as in Figure \ref{fig:avgcurvature}(a).
...
\begin{figure}[ht]
 ...
 \begin{tikzpicture}[>=latex,every node/.style={font=\small}]
  ...
  \draw[name path=t1,red] (A1) -- (A2);
  \draw[name path=t2,red] (B1) -- (B2);
...
For a curve $y=f(x)$, recall from formula (\ref{eqn:tangentangle}) in Section~\ref{sec:tangent-lines}
that $\phi = \phi(x) = \tan^{-1} f'(x)$ at each point
...
\kappa ~=~ \frac{f''(x)}{\left(1 \;+\; \left(f'\left(x\right)\right)^2\right)^{3/2}}
\end{equation}}
...
(\ref{eqn:paramderiv1}) in Section~\ref{sec:parametric-equations}, then by formula (\ref{eqn:paramderiv2}):
\[
...
\newpage
%Begin Section 8.4
\section{Surfaces and Solids of Revolution}\label{sec:surfaces-and-solids-of-revolution}
...
\begin{figure}[ht]
 ...
 \begin{tikzpicture}[>=latex,every node/.style={font=\small}]
  ...
  \draw[thick] (1,0.6) arc (90:270:0.2 and 0.6);
  \draw[densely dashed] (1,0.6) arc (90:-90:0.2 and 0.6);
  \draw[thick] (3,1.2) arc (90:270:0.3 and 1.2);
  \draw[densely dashed] (3,-1.2) arc (270:450:0.3 and 1.2);
 ...
 \begin{tikzpicture}[>=latex,every node/.style={font=\small}]
  ...
  \draw[densely dashed] (D) arc(-90:90:0.24 and 0.815);
  \draw[densely dashed] (C) arc(-90:90:0.24 and 0.75);
  \draw (1,-0.6) parabola (3,-1.2);
  \draw[->,black!60,line width=1pt] (0,-1.2) -- (0,1.5) node[above] {$y$};
  \draw[->,black!60,line width=1pt,anchor=base] (0,0) -- (4,0) node[right] {$x$}
   node[black,shift={(0,-0.4)}] at (1,0) {$a$}
  ...
  \draw[black!60] (1,0.1) -- (1,-0.1);
  \draw[black!60] (3,0.1) -- (3,-0.1);
  \draw[black!60] (1.7,0.1) -- (1.7,-0.1);
  \node at (2.07,0.75) {$\circ$};
  \node[right,inner sep=0,shift={(0.07,0.05)}] at (2.07,0.75) {\tiny$\left(x+\dx,f(x+\dx)\right)$};
  \node at (1.9,0.71) {$\circ$};
  \node[above left,inner sep=0] at (1.9,0.71) {\tiny$\left(x,f(x)\right)$};
  \fill (1,0.6) circle (2.5pt);
  \fill (3,1.2) circle (2.5pt);
  \node[left] at (2.3,1.2) {$y=f(x)$};
 \end{tikzpicture}}
 ...
 \begin{tikzpicture}[>=latex,every node/.style={font=\small}]
  \draw[|<->|] (0.7,0) -- (0.7,0.9) node[fill=white,midway,inner sep=0.5mm] {$f(x)$};
  \draw[|<->|] (1.8,0) -- (1.8,1.1) node[fill=white,midway,inner sep=0.5mm,xshift=0.4cm] {$f(x+\dx)$};
  \draw[->,black!60,line width=1pt] (0,-1.2) -- (0,1.5) node[above] {$y$};
...
\begin{tikzpicture}[>=latex,every node/.style={font=\small},scale=1.5]
 ...
 \draw[dashdotted] (1.5,0) arc (0:180:1 and 0.3);
...
the frustrum in Figure \ref{fig:frustrum}:
$\pi\,(r_1+r_2)\,l$.\footnote{One could also imagine cutting the
frustrum vertically and flattening it to get a trapezoid of height
$l$, and side-lengths (from the circumference of the circles)
$2 \pi r_1$ and $2 \pi r_2$ and using the formula for the area of
a trapezoid, $\frac{1}{2}(s_1+s_2)h$, to retrieve this
formula.}
Use that formula with $r_1=f(x)$, $r_2=f(x+\dx)=f(x)+\dy$, and
$l=\ds=\sqrt{1 + (f'(x))^2}\,\dx$ (by formula (\ref{eqn:arclength}) in Section~\ref{sec:arc-length-and-curvature})
as in Figure \ref{fig:surfarea}(c), so that $d\!S$ is
\begin{align*}
d\!S ~&=~ \pi\,(f(x) + (f(x)+\dy))\,\sqrt{1 + (f'(x))^2}\,\dx\\
&=~ 2\,\pi\,f(x)\,\sqrt{1 + (f'(x))^2}\,\dx ~+~ \pi\,\sqrt{1 + (f'(x))^2}\,\dy\,\dx\\
&=~ 2\,\pi\,f(x)\,\sqrt{1 + (f'(x))^2}\,\dx ~+~ 0 \qquad\left(\because \dy\,\dx = f'(x)\,(\dx)^2 = 0\right)
\end{align*}
...
\begin{figure}[ht]
 ...
 \begin{tikzpicture}[>=latex,every node/.style={font=\small}]
  \draw[|<->|] (0.7,0) -- (0.7,1.5) node[fill=white,inner sep=0.5mm,midway] {$f(x)$};
  \draw[|<->|] (2.2,0) -- (2.2,2.1) node[fill=white,inner sep=0.5mm,pos=0.3,xshift=0.3cm] {$f(x+\dx)$};
  \fill[black!30] (1.05,0) -- (1.05,1.5) -- (1.5,1.5) -- (1.5,0) -- cycle;
  \draw[densely dashed] (1.05,1.5) -- (1.5,1.5);
...
which might not always be the $x$-axis. The idea is to find the radius $r$ and height
...
\begin{figure}[ht]
 ...
 \begin{tikzpicture}[>=latex,every node/.style={font=\small}]
  \draw[|<->|] (0.7,0) -- (0.7,1.5) node[fill=white,inner sep=0.5mm,midway] {$f(x)$};
  \draw[|<->|] (2.2,0) -- (2.2,2.1) node[fill=white,inner sep=0.5mm,pos=0.3,xshift=0.2cm] {$f(x+\dx)$};
  \fill[black!30] (1.05,0) -- (1.05,1.5) -- (1.5,1.5) -- (1.5,0) -- cycle;
  \draw[densely dashed] (1.05,1.5) -- (1.5,1.5);
  \draw (1.05,0) -- (1.05,1.5);
  \draw (1.5,0) -- (1.5,2.1);
...
and $h$ is the height of the strip: $V ~=~ \int_a^b d\!V ~=~ \int_a^b
2\,\pi\,\underbracket[0.3pt]{x}_{r}\,\underbracket[0.3pt]{\Abs{f(x)}}_{h}~\underbracket[0.3pt]{\dx}_{w}$.
\vspace{-0.8em}
...
By the same arguments used in Section~\ref{sec:surfaces-and-solids-of-revolution}, all the area from that strip comes
...
position of its center of gravity relative to the $x$-axis (i.e. the $y$
coordinate of its center of gravity):
\[
m_x ~=~ \underbracket[0.3pt]{\left(f_1(x)-f_2(x)\right)\,\dx}
_{\begin{minipage}[c]{0.2\linewidth}
    \tiny
    The area of the lamina is its mass.
\end{minipage}}
\,\cdot\quad\left(\tfrac{1}{2}\,\left(f_1(x)+f_2(x)\right)\right) ~=~
\tfrac{1}{2}\,\left(\left(f_1(x)\right)^2 - \left(f_2(x)\right)^2\right)\,\dx
\]
...
\begin{align*}
d\!W ~&=~ \frac{1}{2}\,\left(F(x) + F(x+\dx)\right)~\dx ~=~
\frac{1}{2}\,\left(F(x) + F(x)+ \df\right)~\dx\\[4pt]
&=~ \frac{1}{2}\,\left(F(x) + F(x)+ F'(x)\,\dx\right)~\dx ~=~ F(x)\,\dx ~+~ \frac{1}{2}\,F'(x)\,\cancelto{0}{(\dx)^2}\\
&=~ F(x)\,\dx
\end{align*}
...
N, but they have the same magnitude $\abs{\pm 3}=3$.\index{vector}\index{scalar}
...
\begin{enumerate}[\bfseries (a)]
\itemsep0em
\item The force required to stretch the spring by an
...
50$ N/m.
\item By part (a) the force required to compress the string to position $x$ is
...
probability of the \textbf{event}\index{event} that $X$---the number of heads that result when two evenly balanced pennies are flipped---equals the number $x$.
...
$<$ and $\le$ (as well as $>$ and $\ge$) are interchangeable for events involving continuous random
variables. In the remainder of this section it will
