\chapter{Methods of Integration}\label{ch:methods-of-integration}
%Begin Section 6.1
\section{Integration by Parts}\label{sec:int-by-parts}
...
&=~ -x\,e^{-x}\,\dx ~+~ e^{-x}\,\dx\\
&=~ -x\,e^{-x}\,\dx ~-~ d(e^{-x})\\
\Rightarrow~\,e^{-x}\,\dx ~&=~ -d(x\,e^{-x}) ~-~ d(e^{-x}) \quad(\text{Now integrate both sides})\\
\Rightarrow~\int x\,e^{-x}\,\dx ~&=~ -\int d(x\,e^{-x}) ~-~ \int d(e^{-x})
~=~ -x\,e^{-x} ~-~ e^{-x} ~+~ C \quad\left(\because~\int d\!F \,=\, F \,+\, C\right)\\
\text{Generalizing}~&\text{this process for functions $u$ and $v$,}\\
d(uv) ~&=~ u\,\dv ~+~ v\,\du\\
u\,\dv ~&=~ d(uv) ~-~ v\,\du\\
\text{and integrating}&~\text{both sides yields the
  \textbf{integration by parts} formula:\index{integration!by parts}}
\end{align*}
}
\vspace{-2em}
...
integral and can be integrated easily (using a substitution as in Example~\ref{exmp:subst4}).
...
$\du = 2x\,\dx$ and $v = \int \dv = \int e^{-x}\,\dx = -e^{-x}$. Now integrate by parts:
\begin{align*}
\int u\,\dv ~&=~ uv ~-~ \int v\,\du\\[6pt]
...
you know from Section~\ref{sec:higher-order-derivatives} that its $(n+1)$-st derivative will be 0, at which
...
2\,\int \sec^3 x~\dx ~&=~ \sec\,x\;\tan\,x ~+~
 \ln\;\abs{\,\sec\,x \;+\; \tan\,x\,} \quad\left(\because~\text{Example}~\ref{exmp:subst9}\right)\\[6pt]
...
simplified with the useful product-to-sum formulas:\footnote{See Section~\ref{sec:implicit-differentiation} in
...
\sin\,x~\sin\,12x ~&=~ -\tfrac{1}{2}\;(\cos\;(x+12x) ~-~ \cos\;(x-12x))\\
                   &=~ -\tfrac{1}{2}\;(\cos\,13x ~-~ \cos\,11x)\qquad\left(\because~\cos(-x)=\cos(x)\right)
\end{align*}
Then,
\begin{align*}
\int 0.5\,\sin\,x\;\sin\,12x~\dx ~&=~ -\frac{1}{4}\,\int (\cos\,13x ~-~ \cos\,11x)~\dx\\[6pt]
&=~ -\frac{1}{4} \left(\frac{\sin\,13x}{13}-\frac{\sin\,11x}{11}\right)\\
...
\CRef{thm:powerint}{Power Formula} to integrate that polynomial.
...
\ref{exmp:intparts7} in Section~\ref{sec:int-by-parts}:
...
using the \CRef{thm:powerint}{Power Formula} with the substitution $u=1+x^2$ (why?). Integration by
...
&=~ \frac{\theta}{2} ~+~ \frac{1}{2}\,\sin\,\theta\;\cos\,\theta ~+~ C\quad\left(\because~\sin\,2\theta = 2\,\sin\,\theta\;\cos\,\theta\right)
\end{align*}
...
$\tan\,\theta = x = \frac{x}{1}$ (to undo the trigonometric substitution made earlier), as in the drawing on the right. The hypotenuse
...
\CRef{thm:powerint}{Power Formula}, so try a trigonometric substitution. Complete the square on the
...
triangle with an angle $\theta$ such that $\sec\,\theta = \frac{2(x+1)}{3}$ (to undo the trigonometric substitution made earlier), as
...
The following integrals from Section~\ref{sec:int-by-sub} might be helpful for the exercises:
...
\phantomsection\label{cmnt:part-frac-case-1}
...
\phantomsection\label{cmnt:part-frac-case-2}
...
$B_n$. Those constants can be solved for by the same method as in \CRef{cmnt:part-frac-case-1}{Case 1}.}
...
\CRef{cmnt:part-frac-case-2}{Case 2} can be extended to more than one repeated factor---the partial
...
\par\noindent\emph{Solution:} Expanding \CRef{cmnt:part-frac-case-2}{Case 2} to two repeated factors,
\begin{align*}
...
\phantomsection\label{cmnt:part-frac-case-3}
...
so by \CRef{cmnt:part-frac-case-3}{Case 3},
\begin{align*}
...
\end{align*}
by formula (\ref{eqn:atanint}) in Section~\ref{sec:int-by-sub}.
\end{exmp}
\divider
\vspace{2mm}
...
\phantomsection\label{cmnt:part-frac-case-4}
...
solved for by the same method as in \CRef{cmnt:part-frac-case-1}{Case 1}.}
\newpage
\begin{exmp}\label{exmp:partfrac5}
\noindent Evaluate $~\displaystyle\int \frac{\dx}{(x^2+1)^2\,(x^2+4)}~$.\vspace{1mm}
\par\noindent\emph{Solution:} Neither $x^2 + 1$ nor $x^2 + 4$ has real roots,
and $x^2 + 1$ is repeated, so by \CRef{cmnt:part-frac-case-4}{Case 4},
\begin{align*}
...
&=~ -\frac{1}{9}\,\tan^{-1} x ~+~
     \frac{1}{3}\,
     \underbracket[0.3pt]{\left(\frac{1}{2}\,\tan^{-1} x ~+~ \frac{x}{2\,(x^2 + 1)}\right)}
     _{\begin{minipage}[t]{0.194\linewidth}
         \tiny
         From Example \ref{exmp:trigsub2} in Section~\ref{sec:trig-sub}
     \end{minipage}}
     ~+~
...
\newpage
%Begin Section 6.5
\section{Miscellaneous Integration Methods}\label{sec:misc-integration-methods}
...
Recall from \CRef{thm:inteax}{the generalization} of Example~\ref{exmp:subst2} in Section~\ref{sec:int-by-sub}, that
\begin{equation}
\int e^{\alpha x}\;\dx ~=~ \tfrac{1}{\alpha}\,e^{\alpha x} ~+~ C\quad\left(\text{for any constant $a \ne 0$}\right)\label{eqn:diffinteax}
\end{equation}
This antiderivative involves the variable $x$
...
Company, Inc., 1939.}
\begin{equation}\label{leibniz-rule:first-example-RHS}
\underbracket[0.3pt]{
\frac{d}{\dalpha} \int e^{\alpha x}\;\dx ~=~
\int \frac{d}{\dalpha}\,(e^{\alpha x})~\dx
}_{\begin{minipage}[t]{0.3\linewidth}
    \scriptsize
    This closely parallels the following immediate consequence of the
    definition of the anti-derivative (see
    Section~\ref{sec:indefinite-integral}):\\
    Since $\ddx\,\left(\int\,f(x)\,\dx\right) = f(x)$\\
    and $\int\,f'(x)\,\dx = f(x) + C$,\\
    $\therefore~\ddx\,\left(\int\,f(x)\,\dx\right) = \int\,\left(\frac{d}{\dx}\,f(x)\right)\,\dx - C$
\end{minipage}}
~=~ \int x\,e^{\alpha x}\;\dx
\end{equation}
On the other hand, differentiating \emph{through} equation
(\ref{eqn:diffinteax}) with respect to $\alpha$ shows that
\begin{equation}\label{leibniz-rule:first-example-LHS}
\underbracket[0.3pt]{
\frac{d}{\dalpha} \int e^{\alpha x}\;\dx ~=~
\frac{d}{\dalpha} \left(\tfrac{1}{\alpha}\,e^{\alpha x} ~+~ C\right)
}_{\begin{minipage}[t]{0.335\linewidth}
    \tiny
    $d/\dalpha$ through RHS of (\ref{eqn:diffinteax}) rather than LHS,
    as in (\ref{leibniz-rule:first-example-RHS}).
\end{minipage}}
~=~
\underbracket[0.3pt]{
  \frac{\alpha\,\left(x\,e^{\alpha x}\right) ~-~ 1\,\cdot\,e^{\alpha x}}{\alpha^2}
}_{\begin{minipage}[t]{0.185\linewidth}\tiny Product Rule (with respect to $\alpha$)\end{minipage}}
~=~
\tfrac{1}{\alpha}\,x\,e^{\alpha x} ~-~ \tfrac{1}{\alpha^2}\,e^{\alpha x}
\end{equation}
Thus, since (\ref{leibniz-rule:first-example-RHS}) and
(\ref{leibniz-rule:first-example-LHS}) both equal $\frac{d}{\dalpha}
\int e^{\alpha x}\;\dx$, the corresponding final terms imply that
\[
\int x\,e^{\alpha x}\;\dx ~=~
...
\par\noindent\emph{Solution:} By formula (\ref{eqn:atanint}) in Section~\ref{sec:int-by-sub},
\[
...
\]
which agrees with the result from Example \ref{exmp:trigsub2} in Section~\ref{sec:trig-sub}.\\
...
integral is convergent, since
\[
\underbracket[0.3pt]{e^{x^2} ~\ge~ 1 ~+~ x^2}
_\text{\tiny See Exercise~\ref{exer:exple1px} in Section~\ref{sec:mvt}}
\quad\Rightarrow\quad
\frac{1}{e^{x^2}} ~\le~ \frac{1}{1 ~+~ x^2}
\quad\Rightarrow\quad
0 ~\le~ e^{-x^2} ~\le~ \frac{1}{1 ~+~ x^2}
\qquad\text{for all $x$}
\]
implies $I$ is convergent by \CRef{thm:impropercomptest}{the Comparison Test} from Section~\ref{sec:improper-integrals}, as
$\int_0^{\infty} \frac{1}{1 + x^2}\,\dx$ is convergent (and equals
$\tfrac{1}{2}\pi$) by Example \ref{exmp:improper5} in Section~\ref{sec:improper-integrals}.
\begin{flalign*}
\text{Now, for $\alpha \ge 0$, define:}\qquad\qquad\qquad\qquad\phi(\alpha) ~=~ \int_0^{\infty} \,\frac{\alpha\,e^{-\alpha^2 x^2}}{1 + x^2} \,\dx&&
\end{flalign*}
Then clearly $\phi(0) = 0$, and differentiating under the integral sign shows
\[
\phi'(\alpha) ~=~ \int_0^{\infty} \,\frac{-2 x^2 \alpha^2 e^{-\alpha^2 x^2} + e^{-\alpha^2 x^2}}{1 + x^2}~\dx
\quad\Rightarrow\quad
...
&=~ -2\alpha\, e^{-\alpha^2}\,\int_0^{\infty} e^{-\alpha^2 x^2}~\dx \qquad\qquad\quad(\text{now
substitute $u = \alpha x$ and $\du = \alpha \dx$ to get})\\[6pt]
&=~ -2\alpha\, e^{-\alpha^2}\,\frac{1}{\alpha}\,\int_0^{\infty} e^{-u^2} \,\du
~=~ -2\, e^{-\alpha^2}\,I \qquad(\text{and so integrating both sides yields})\\[6pt]
...
&=~ 0 ~-~ \left(\lim_{\alpha \to 0}~\frac{\phi(\alpha)}{\alpha\,e^{\alpha^2}}\right)
 ~\to~ \frac{0}{0} \qquad\qquad\qquad\qquad\qquad(\text{so by L'H\^{o}pital's Rule})\\[6pt]
&=~ -\lim_{\alpha \to 0}~\frac{\phi'(\alpha)}{e^{\alpha^2} + 2\alpha^2\,e^{\alpha^2}}
~=~ -\frac{\phi'(0)}{1+0} ~=~ -\tfrac{1}{2}\pi
\end{align*}
Thus,
\[
-2I^2 ~=~ -\tfrac{1}{2}\pi \quad\Rightarrow\quad I ~=~ \tfrac{1}{2}\sqrt{\pi}
\]
...
Section~\ref{sec:higher-order-derivatives} that the zero-th derivative of a function is just the function
...
\quad\left(\because~\Gamma\,\left(\tfrac{1}{2}\right) ~=~ \sqrt{\pi}
~\text{by Example \ref{exmp:intgamma1}}\right)
\]
Use the substitution $u=\sqrt{x-t}$,
...
\]
\newpage
Recall from Section~\ref{sec:trig-sub} that the trigonometric substitution
...
(Equation~\ref{eqn:halftan1}) directly, since
\[
\int \frac{\sin\,\theta}{1 \;+\; \cos\,\theta}\,\dtheta ~=~
\int \tan\,\tfrac{1}{2}\theta~\dtheta ~=~
2\,\ln\,\Abs{\sec\,\tfrac{1}{2}\theta} ~+~ C
\]
by formula (\ref{eqn:inttanu}) in Section~\ref{sec:trig-sub}.
\end{exmp}\vspace{-2mm}
\divider
\newpage
...
\]
 \item Use Exercise \ref{exer:gamma} from Section~\ref{sec:int-by-parts}, as well as Exercise
...
\]
 \item Using the result from Exercise \ref{exer:eaxtrigbx}  in Section~\ref{sec:int-by-parts} that
\[
...
\probs{C}
\resume{enumerate}[{[\bfseries 1.]}]
 \item Use the Leibniz rule and formula (\ref{eqn:sqrta2u2tan}) from Section~\ref{sec:trig-sub}
% \[
...
Section~\ref{sec:definite-integral} showed how to obtain exact values for definite integrals of some
...
New York: Quantum Publishers, Inc., 1975. or \S\,1.11 `Numerical
Integration' in \textsc{Feldman, J., A. Rechnitzer and E. Yeager},
\emph{CLP-2 Integral Calculus},
