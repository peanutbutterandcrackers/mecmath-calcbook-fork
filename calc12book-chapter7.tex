...
\parpic[r]{\begin{tikzpicture}[>=latex,every node/.style={font=\small}]
 ...
 \node[below left] at (-2.2,-0.1) {$(-a,0)$};
 \fill (2.2,0) circle (2.5pt);
 \node[below right] at (2.2,-0.1) {$(a,0)$};
 \fill (0,1.2) circle (2.5pt);
 \node[above left] at (0,1.2) {$(b,0)$};
 \fill (0,-1.2) circle (2.5pt);
 \node[below left] at (0,-1.2) {$(-b,0)$};
\end{tikzpicture}}
In the $xy$-plane, let the foci of an ellipse be the points $(\pm
c,0)$ for some $c>0$, so that the center is the origin $(0,0)$ and the
$x$-axis is the principal axis, as in the figure on the right. Denote
by $(\pm a,0)$ with $a > 0$ the vertexes, by $b$ the length of the
semi-major axes, and by $k$ the constant sum $d_1+d_2$ of the
distances from points $(x,y)$ on the ellipse to the foci. Notice that
$a>c$. Then by the distance formula,
\begin{align*}
d_1 ~+~ d_2 ~&=~ k\\
\sqrt{(x+c)^2 + y^2} ~+~ \sqrt{(x-c)^2 + y^2} ~&=~ 2a
~\left(~
\begin{minipage}[c]{0.567\linewidth}
  \small
  Since $(a,0)$ is a point on the ellipse, it must satisfy $d_1 ~+~
  d_2 ~=~ \sqrt{(x+c)^2 + y^2} ~+~ \sqrt{(x-c)^2 + y^2} ~=~ k$
  i.e. $\sqrt{(a+c)^2 + 0^2} ~+~ \sqrt{(a-c)^2 + 0^2} = k
  \allowbreak\Rightarrow \abs{(a+c)} ~+~ \abs{a-c} = k
  \allowbreak\Rightarrow (a+c) ~+~ (a-c) = k ~(\because a > c)
  \allowbreak\Rightarrow 2a = k$.
\end{minipage}
~\right)\\
...
\begin{minipage}[c]{0.58\linewidth}
  \small
  Since $(0,b)$ is a point on the ellipse, it must satisfy $d_1 ~+~
  d_2 ~=~ \sqrt{(x+c)^2 + y^2} ~+~ \sqrt{(x-c)^2 + y^2} ~=~ k ~=~ 2a$
  i.e. $ \sqrt{(0+c)^2 + b^2} ~+~ \sqrt{(0-c)^2 + b^2} = 2a
  \allowbreak\Rightarrow \cancel{2}\sqrt{c^2 + b^2} = \cancel{2}a
  \allowbreak\Rightarrow \abs{c^2 + b^2} = a^2
  \allowbreak\Rightarrow c^2 + b^2 = a^2
  \allowbreak\Rightarrow b^2 = a^2 - c^2$.
\end{minipage}
~\right)\\
\frac{x^2}{a^2} + \frac{y^2}{b^2} ~&=~ 1
\quad\text{$\because~b^2 = a^2 - c^2$ (and so $a > b > 0$)}\quad\checkmark
\end{align*}
\newpage
...
\[
y^2 ~=~ b^2\left(1 ~-~ \frac{x^2}{a^2}\right) \quad\Rightarrow\quad
y ~=~ b\,\sqrt{1 - \frac{x^2}{a^2}} ~=~ \frac{b}{a}\,\sqrt{a^2 - x^2}
\]
for the upper hemisphere. Thus,
\begin{align*}
\text{Area} ~&=~ 4\,\int_0^a y(x)~\dx ~=~ \frac{4b}{a}\,\int_0^a \sqrt{a^2 - x^2}~\dx\\[6pt]
&=~ \frac{4b}{a}\,\left(\frac{a^2}{2}\,\sin^{-1} \left(\frac{x}{a}\right) ~+~
 \frac{1}{2}\,x\,\sqrt{a^2 - x^2} ~\Biggr|_0^a\right)
 \quad\text{(by formula (\ref{eqn:sqrta2u2sin}) in Section~\ref{sec:trig-sub})}\\[6pt]
...
\ref{exmp:minmax4} in Section~\ref{sec:optimization} showed that the incoming angle $\theta_1$
...
By Exercise \ref{exer:elliptan} in Section~\ref{sec:implicit-differentiation}, the equation of the tangent line
...
(F_1P)^2 ~&=~ x_0^2 ~+~ 2eax_0 ~+~ e^2a^2 ~+~ (1-e^2)\,(a^2 - x_0^2)
...
\frac{\sin\,(\pi - \theta)}{F_2P} ~=~
\frac{\sin\,(180\Degrees - \theta)}{F_1P}\,\cdot\,\frac{F_1P}{F_2P} ~=~
\frac{\sin\,\alpha_1}{F_1N}\,\cdot\,\frac{F_1P}{F_2P} ~=~
...
in Exercise \ref{exer:ellipdirectrix} in Section~\ref{sec:ellipses} is, in fact, similar to the
...
\end{tikzpicture}}
\par\noindent\emph{Solution:} Recall from Equation~\ref{eqn:exmp_minmax3} in Example~\ref{exmp:minmax3} in Section~\ref{sec:optimization}
that if the object is launched at an angle $0<\theta<\frac{\pi}{2}$ with the
ground, then the height $y$ attained by the object as a function of the
horizontal distance $x$ that it travels is
\[
...
Section~\ref{sec:indefinite-integral}. By symmetry only the angles $0<\theta\le\frac{\pi}{2}$ in the same
...
would appear to have a parabolic boundary. This will now be shown to be true.
...
\ref{exer:projmaxangle} in Section~\ref{sec:optimization} that the maximum height reached by the
object is $\frac{v_0^2\,\sin^2 \theta}{2g}$, which is thus the $y$-coordinate of
the vertex of each of the parabolas. That vertex is midway between the focus and the
directrix. Each of the parabolas are of the form $4py = x^2 + bx$, where $b$ is a constant
that does not affect the distance between the vertex and directrix\footnote{This
will be discussed further in Section~\ref{sec:translations-and-rotations} (Example~\ref{exmp:ptrans1}).}, and $4p$ is a constant with $p<0$
such that the directrix is $-p$ units above the vertex (since $p<0$), just as in
the case $4py=x^2$ with $p<0$, as shown in Figure \ref{fig:parabolap}(a). The equation of the parabola then shows that
\begin{align*}
\frac{1}{4p} ~&=~ -\frac{g}{2v_0^2 \cos^2\,\theta} \quad\Rightarrow\quad
p ~=~ -\frac{v_0^2 \cos^2\,\theta}{2g}\\
\text{so that the directrix is at:}\quad y ~&=~ \text{$y$-coordinate of the vertex} ~+~ (-p)\\
&=~ \frac{v_0^2\,\sin^2 \theta}{2g} ~+~ -\left(-\frac{v_0^2 \cos^2\,\theta}{2g}\right) \quad\left(\because~\text{Exercise
\ref{exer:projmaxangle} in Section~\ref{sec:optimization}}\right)\\
&=~ \frac{v_0^2}{2g}(\sin^2 \theta ~+~ \cos^2\,\theta) ~=~ \frac{v_0^2}{2g}
\end{align*}
...
of the proof.
\newpage
\noindent Now let $P$ be a point in the first quadrant below the common directrix
of the parabolic trajectories, $y=\frac{v_0^2}{2g}$, denoted by $D$. Then $P$ can be
...
\noindent The origin $O=(0,0)$ is on each trajectory, at a distance of
$\frac{v_0^2}{2g}$ from the common directrix $D$, so by
\CRef{defn:parabola}{definition of a parabola} the foci for each
trajectory must also be the same distance away from $O$. That is, the
focus of each trajectory must lie \emph{on} the circle $C_0$ of radius
$v_0^2/\left(2g\right)$ centered at $O$. If $P$ is a point inside the
envelope---meaning it lies on at least one trajectory---then it must
be a distance $r>0$ below the line $D$, and---by
\CRef{defn:parabola}{definition of a parabola}---must also be the same
distance $r$ from the focus of the trajectory it belongs to. That is,
the foci must be on a circle $C$ of radius $r$ centered at $P$ and
touching the directrix $D$, as in Figure \ref{fig:envfoci3}:
...
\piccaption[]{\label{fig:hyper1x}}\parpic[r]{\begin{tikzpicture}[>=latex,every node/.style={font=\small}]
  ...
  \end{scope}
 \end{tikzpicture}}
It will be shown in Section~\ref{sec:translations-and-rotations} that the curve $y=\frac{1}{x}$ is a hyperbola,
...
The ratio of distances referred to in the \CRef{defn:hyperbola}{definition of the hyperbola} also
appears in the \CRef{exer:ellipdirectrix}{second definition of the ellipse} (where the ratio is smaller than
...
\piccaption[]{\label{fig:hypereqn}}\parpic[r]{\begin{tikzpicture}[>=latex,every node/.style={font=\small}]
 ...
 \node[right] at (P) {$~(x,y)$};
 \fill (1,0) circle (2.5pt);
 \node[above right] at (1,-0.1) {$(a,0)$};
 \end{tikzpicture}}
To derive the equation of a hyperbola with eccentricity $e>1$ and vertex
(point on the hyperbola closest to the directrix) at $(a,0)$,
assume\footnote{These magic numbers are the simplest ones that satisfy the condition for a hyperbola:
$\frac{d_1}{d_2}=\frac{ea-a}{a-\frac{a}{e}}=\frac{\cancel{ea-a}}{\frac{\cancel{ea-a}}{e}}=e>1$, and are found---if $(f,0)$ is assumed the focus and $x=d$ the directrix (a sketch on a paper, left as an exercise for the reader, will be helpful here)---by solving the system of equations: $\frac{f-a}{a-d}=e$ and $\frac{f}{a}=e$, for focus $f$ and directrix $d$.} the focus
...
x^2 ~-~ \cancel{2eax} ~+~ e^2a^2 ~+~ y^2 ~&=~ e^2x^2 ~-~ \cancel{2eax} ~+~ a^2\\
(e^2 - 1)\,x^2 ~-~ y^2 ~&=~ (e^2 - 1)\,a^2\\
%\frac{x^2}{a^2} ~-~ \frac{y^2}{(e^2 - 1)\,a^2} ~&=~ 1\\[4pt]
\frac{x^2}{a^2} ~-~ \frac{y^2}{b^2} ~&=~ 1, \quad\text{where $b^2 = (e^2 - 1)\,a^2 > 0$.}
\end{align*}
}
\picskip{0}
\vspace{-2em}
...
By Exercise \ref{exer:hyptan} in Section~\ref{sec:implicit-differentiation}, the tangent
...
\begin{align*}
\tan\,\theta_2 ~&=~ \tan\,(\theta - \alpha_1) ~=~ \frac{\tan\,\theta ~-~ \tan\,\alpha_1}{1 ~+~ \tan\,\theta\;\tan\,\alpha_1} ~=~
...
\]
\newpage
\noindent Thus, since $\tan\,\alpha_2 = -\tan\,(\pi - \alpha_2) =
-\tan\,(180\Degrees - \alpha_2) = -\frac{y_0-0}{x_0-c} =
-\frac{y_0}{x_0-c}$ from Figure~\ref{fig:hyperreflpt1},\footnote{ This
agrees with the more straightforward $\tan\,\alpha_2 =
\frac{0-y_0}{c-x_0} = -\frac{y_0}{x_0-c}$.  }
\begin{align*}
\tan\,\theta_1 ~&=~ -\tan\,(\theta + \alpha_2) ~=~ -\frac{\tan\,\theta ~+~ \tan\,\alpha_2}{1 ~-~ \tan\,\theta\;\tan\,\alpha_2} ~=~
 -\frac{\frac{b^2x_0}{a^2y_0} ~+~ \frac{-y_0}{x_0 ~-~ c}}{1 ~-~
 \frac{b^2x_0}{a^2y_0}\;\cdot\;\frac{-y_0}{x_0 ~-~ c}} ~=~
 -\frac{\frac{b^2 x_0^2 ~-~ a^2 y_0^2 ~-~ b^2 x_0 c}{\cancel{a^2 y_0\,(x_0 ~-~
  c)}}}{\frac{(a^2 ~+~ b^2)\,x_0 y_0 ~-~ a^2 y_0 c}{\cancel{a^2 y_0\,(x_0 ~-~ c)}}}\\[6pt]
...
\piccaption[]{\label{fig:spheretan}}\parpic[r]{\begin{tikzpicture}[every node/.style={font=\small}]
 ...
 \draw[densely dashdotted,line width=0.2pt] (1.25,0) arc (0:180:1.25 and 0.3);
 \fill (0,0) circle (2.5pt);
 \node[right] at (0,0) {$O$};
 ...
 \draw (225:1.25) -- ++(135:1.25) node[left] {$Q$};
 \draw (135:1.25) -- ++(225:1.25);
 \draw [densely dotted] (0,0) -- (225:1.25);
 \draw [dashed] (0,0) -- (180:1.7);
 \draw [densely dotted] (0,0) -- (135:1.25);
...
\piccaption[]{\label{fig:conicproof}}\parpic[r]{\begin{tikzpicture}[every node/.style={font=\small}]
 ...
 \draw[densely dashdotdotted,shift={(0,1)}] (1.732,0) arc (0:180:1.732 and 0.3);
 \draw[ultra thick] (4,1.5) -- (3,0.5);
 \draw (-3.5,0.5) -- (3,0.5) -- (4,1.5) node[pos=0.8,right] {$\boldsymbol{D}$} -- (-1.5,3.4)
  -- (-2.5,2.4) -- (3,0.5);
 ...
 \fill (B) circle (2.5pt);
 \draw pic ["\scriptsize$\alpha$", fill=blue!30, angle radius=7.0mm, angle eccentricity=0.85, draw, -] {angle=P--B--A};
 ...
 \draw pic ["\tiny$\beta$", fill=blue!30, angle radius=2.7mm, draw, -] {angle=A--Q--P};
 \fill (Q) circle (2.5pt);
 \draw pic [fill=blue!30, angle radius=0.8mm, draw] {right angle=Q--A--P};
\end{tikzpicture}}
For the case of a right circular double cone\footnote{The proof can be
extended to oblique double cones. See \S 364 in \textsc{Salmon, G.S.},
\emph{A Treatise on Conic Sections}, London: Longmans, Green and Co.,
1929.} (i.e. the circular base of each nappe is in a plane
...
\newpage
%Begin Section 7.4
\section{Translations and Rotations}\label{sec:translations-and-rotations}
...
\piccaption[]{\label{fig:translation}\enskip Translation}\parpic[r]{
 \begin{tikzpicture}[>=latex,every node/.style={font=\small}]
  ...
  \node[right,align=left] at (2.8,2.4) {
    {\small
  ...
  \draw[<->,densely dashed] (1,2.4) -- (2.8,2.4) node[midway,fill=white,inner sep=0mm] {$x-h$};
  \draw[<->,densely dashed] (2.8,2.4) -- (2.8,1) node[midway,fill=white,inner sep=0mm] {$y-k$};
...
\piccaption[]{\label{fig:rotation}\enskip Rotation}\parpic[r]{
 \begin{tikzpicture}[>=latex,every node/.style={font=\small}]
  ...
  \node[above right,align=left] at (45:2.5) {
    {\addtolength{\jot}{-0.4em}
      $\begin{aligned}
        P~&=(x,y)\\
        P'&=(x',y')\\
...
needed:\footnote{Found by switching the coordinate systems and making
the substitution $\theta \mapsto -\,\theta$ in the original equations.}
\begin{align}
...
half-angle identity (see Equation~\ref{eqn:halftan1}, Section~\ref{sec:misc-integration-methods}) to find $\theta$:
\[
...
\end{multicols}
 ...
 \item\label{exer:hypdx} Show that $\sinh\,\dx = \dx$ and $\cosh\,\dx=1$ for any
  ...
  Exercise \ref{exer:1over1plusdx} in Section~\ref{sec:derivative-inifinitesimal-approach} along with $e^{\dx}=1+\dx$
  from Exercise \ref{exer:expdx} in Section~\ref{sec:exponential-and-natural-logarithm-functions}.)}
...
hyperbola in Figure \ref{fig:hyprotate}(c). Then:
 \begin{enumerate}[\bfseries (a)]
  \item Let $c>0$. When $c \ne 1$ the mapping $\phi: (x,y) \mapsto (cx,y/c)$
   ...
   use the rotation equations from Section~\ref{sec:translations-and-rotations} to rotate the
...
Recall that Section~\ref{sec:misc-integration-methods} presented two different ways to ``identify'' or
...
hyperbola $x^2-y^2=1$. This was in fact shown in Section~\ref{sec:hyperbolic-functions}, where $t$ is twice
...
via the \CRef{thm:chainrule}{Chain Rule}:
...
(\ref{eqn:paramderiv1}) and (\ref{eqn:paramderiv2}) from Section~\ref{sec:parametric-equations}, with
...
required when using Cartesian coordinates, as in Section~\ref{sec:trig-sub}. Notice also that
