\chapter{Derivatives of Common Functions}\label{ch:derivatives-of-common-functions}
%Begin Section 2.1
...
proved in Chapter~\ref{ch:topics-in-differential-calculus}).
...
true for the infinitesimals $\dy$ and $\dx$ (which are nonzero \emph{by definition}) since they
...
$\dxdy = \frac{1}{\dydx}$ will be in terms of $y$. That might not always be
...
\[
 ...
 \quad\big(\text{ $\because$ Chain Rule}\big)
\]
and hence:
...
\intertext{and}
\left(f^{-1}\right)'(x) ~&=~ \frac{1}{f'(f^{-1}(x))} \quad\text{if $~f'(f^{-1}(x)) \ne 0$}
\quad
\left(
\because
~
\begin{minipage}[c]{0.4\linewidth}
  $f(x)=y \Rightarrow f^{-1}(f(x))=f^{-1}(y)=x$, and switch the roles
  of $x$ and $y$ in the resulting $\left(f^{-1}\right)'(y) ~=~
  \frac{1}{f'(f^{-1}(y))}$.
\end{minipage}
~
\right)
...
\newpage
%Begin Section 2.2
\section{Trigonometric Functions and Their Inverses}\label{sec:trig-and-inv-trig-functions}
...
\begin{displaymath}
 ...
 \sin y ~=~ \pm\,\sqrt{1 - x^2} ~=~ \sqrt{1 - x^2} \quad\left(\because 0 \le y \le \pi \Rightarrow \sin y \ge 0\right)
\end{displaymath}
\begin{displaymath}
 \text{Thus:}\quad\ddx\,(\cos^{-1} x) ~=~ \dydx ~=~ \frac{1}{\dxdy} ~=~ \frac{1}{-\sin y} ~=~
  -\,\frac{1}{\sqrt{1 - x^2}} \quad\checkmark
 \end{displaymath}

\noindent For the derivative of $\sec^{-1} x$, since $y = \sec^{-1} x$
is defined for $\abs{x} \ge 1$, and since $x = \sec y = \frac{1}{\cos
  y}$, from the unit-circle: $x \ge 1 \Rightarrow 0 \le y < \pi/2$ and
$x \le -1 \Rightarrow \pi/2 < y \le \pi$.  Recall also that $\sec y$
and $\tan y$ are both positive when
...
\newpage
%Begin Section 2.3
\section{The Exponential and Natural Logarithm Functions}\label{sec:exponential-and-natural-logarithm-functions}
...
\vdots \qquad ~&~ \qquad \vdots ~&~ \vdots \qquad \qquad\\
...
The limit in this definition means that as $x$ becomes arbitrarily large---approaching
...
by making $x$ sufficiently large.\footnote{It will be shown in Chapter~\ref{ch:infinite-sequences-and-series} that
...
\begin{displaymath}
 ...
 1 \quad\Rightarrow\quad \left(e^{1/x} ~-~ 1\right)\,\frac{1}{\left(\frac{1}{x}\right)} ~\approx~
 1 ~,
\end{displaymath}
so letting $h = 1/x$, and noting that $h = 1/x \to 0$ if and only if
$x \to \infty$, yields the useful limit:\footnote{This is admittedly a
``hand waving'' argument. In Chapter~\ref{ch:topics-in-differential-calculus} a more exact method will be discussed for
proving limits like this one.}
\statethm{thm:limeh}{\begin{equation}\label{eqn:elimh0}
\lim_{h \to 0}~\frac{e^h ~-~ 1}{h} ~=~ 1
\end{equation}}
Using the above limit---which is true for all $h \in \Reals$ since
$\lbrace h: h = \left(\frac{1}{x}\right), x \in \Reals \rbrace = \Reals$ (as all real
numbers have a multiplicative inverse)---the derivative of $y = e^x$
can be found:
...
\begin{align*}
 ...
 ~=~ e^x \;\cdot\; \underbrace{\lim_{h \to 0}~\frac{e^h ~-~ 1}{h}}_\text{= 1 by equation~\ref{eqn:elimh0}}
     \quad\text{(since $e^x$ does not depend on $h$)}\\
 &=~ e^x \;\cdot\; 1 ~=~ e^x
\end{align*}
\vspace{-3mm}
...
\begin{exmp}\label{exmp:deriv4ex2}
 ...
 \par\noindent\emph{Solution:} $\dydx ~=~ 4 \;\cdot\; e^{-x^2} \;\cdot\; \ddx\,(-x^2) ~=~
 4 \;\cdot\; e^{-x^2} \;\cdot\; (-2x) ~=~ -8x e^{-x^2}$
\end{exmp}
\divider
\newpage
...
\begin{displaymath}
 \dydt ~=~ \ddt\,\left(Ae^{kt}\right) ~=~ A \cdot e^{kt} \cdot k ~=~ k \cdot Ae^{kt} = ky ~,
\end{displaymath}
...
shortly. Conversely, in Chapter~\ref{ch:integral} it will be shown that any solution to the
...
\ddx\,\left(\ln\,u\right) ~=~ \frac{1}{u} \;\cdot\; \dudx ~=~ \frac{1}{u} \;\cdot\; u' ~=~ \frac{u'}{u}
\]}
...
\begin{displaymath}
 ...
 \frac{d\negmedspace I}{\dt} ~=~ \left(-\frac{1}{RC}\right) \;\cdot\; I
\end{displaymath}
so that $I(t) = I_0 e^{-t/RC}$ where $I_0$ is the initial current at $t = 0$.
\emph{Ohm's Law} says that $V = I_0R$, so
\begin{displaymath}
I(t) ~=~ \left(\frac{V}{R}\right) e^{\left(-\frac{1}{RC}\right)t} ~=~ \frac{V}{R} e^{-t/RC}
\end{displaymath}
...
\begin{displaymath}
 \frac{d\negmedspace p}{d\!h} ~=~ \left(-\frac{w_0}{p_0}\right)\,p
\end{displaymath}
...
\begin{displaymath}
 p(h) ~=~ p_0\;e^{\left(-\frac{w_0}{p_0}\right)\,h}~.
\end{displaymath}
...
\begin{align*}
 ...
 \ddx\,(\ln\,y) ~&=~ \ddx\,(x \,\ln\,a) ~=~ \ln\,a ~\left(\because \ln\,a \in \Reals\right)\\
...
In general, for an exponent of the form $u = u(x)$, by the Chain Rule:
...
\newpage
\statethm{thm:derivloga}{\[ \ddx\,\left(\log_a x\right) ~=~ \frac{1}{\,\ln\,a} \;\cdot\; \frac{1}{x} ~=~ \frac{1}{x \,\ln\,a} \]}
In general, when taking the logarithm of a function $u = u(x)$, by the Chain Rule:
...
\resume{enumerate}[{[\bfseries 1.]}]
 \item Show that for all constants $k$ the function $y = A a^{\frac{kx}{\ln\,a}}$
  ...
  statement made in Section~\ref{sec:exponential-and-natural-logarithm-functions} that the only solution to that differential
