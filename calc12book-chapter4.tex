\chapter{Applications of Derivatives}\label{ch:appl-of-derivs}
%Begin Section 4.1
...
\newpage
Every global maximum (or, minimum) is a local maximum (or, minimum), but not vice versa.
...
respectively.} are the primary interest. The \CRef{thm:evt}{Extreme
  Value Theorem} in Section~\ref{sec:continuity} guarantees the
existence of at least one global maximum and at least one
...
\begin{center}\vspace{-14mm}
 \begin{tikzpicture}[>=latex,every node/.style={font=\small}]
  ...
  %% \draw[thin, dashed, gray] (0,0) grid (8,3);
  \draw[red] (1.13,0.73) -- (1.7,1.3) node{.} node[black,left] {$f'>0$} -- (2.22,1.83);
  \draw[red] (3.28,1.86) -- (3.78,1.45) node{.} node[black,right] {$f'<0$} -- (4.3,1.01);
  \draw[red] (5.82, 0.84) -- (6.27, 0.99) node{.} node[black,right] {$f'>0$} -- (6.97, 1.23);
 \end{tikzpicture}
\end{center}\vspace{-2mm}
Assuming that $f'$ is continuous (which will be the case for all the functions
in this section), this means that $f'=0$ at $x=c_1$, that is, $f'(c_1)=0$.
...
proof requires the Mean Value Theorem, which will be presented in Section~\ref{sec:mvt}.}
...
interval containing $x=0$. Section~\ref{sec:curve-sketching} will present an alternative for when the
...
and hence
\[
A ~=~ xy ~=~ x\,\left(\frac{L}{2} ~-~ x\right) ~=~ \frac{Lx}{2} ~-~ x^2
\]
is now a function of $x$ alone, on the open interval $(0,L/2)$ (since the length
$x$ is always positive). Now find the critical points of $A$:
\[
A'(x) ~=~ 0 \quad\Rightarrow\quad \frac{L}{2} ~-~ 2x ~=~ 0
\quad\Rightarrow\quad x ~=~ \frac{L}{4} ~~\text{is the only critical point}
\]
...
eliminated since
\begin{equation}\label{eqn:exmp:minmax2-h}
...
\quad\Rightarrow\quad r^3 ~=~ \frac{V}{2\pi}
\quad\Rightarrow\quad r ~=~ \sqrt[3]{\frac{V}{2\pi}} ~~\text{is the only critical point}
\end{align*}
...
\[
r^3 ~=~ r \cdot r^2 ~=~ \frac{V}{2\pi} \quad\Rightarrow\quad
2r ~=~ \frac{V}{\pi r^2} ~=~ h \quad\left(\because~\text{Equation}~\ref{eqn:exmp:minmax2-h}\right)~.
\]
...
$g$ due to gravity, so that \CRef{thm:gravity}{the equations of motion
  for the projectile}---as will be derived in
Section~\ref{sec:indefinite-integral}---are:
\begin{align*}
 x ~&=~ (v_0 \cos\,\theta)t\\
...
~\Rightarrow~ y ~=~ -\frac{1}{2}gt^2 ~+~ (v_0 \sin\,\theta)t ~=~ -\frac{1}{2}g\left(\frac{x}{v_0 \cos\,\theta}\right)^2
 ~+~ (v_0 \sin\,\theta) \cdot \frac{x}{v_0 \cos\,\theta} ~=~ -\frac{gx^2}{2v_0^2 \cos^2\,\theta} ~+~ x\frac{\sin\,\theta}{\cos\,\theta}
\label{eqn:exmp_minmax3}
\end{gather}
Since $L$ is the value of $x>0$ that makes $y=0$:
\[
0 ~=~ -\frac{gL^2}{2v_0^2 \cos^2\,\theta} ~+~ L\frac{\sin\,\theta}{\cos\,\theta}
...
\begin{gather*}
L'(\theta) ~=~ 0 ~\Rightarrow~ \frac{2v_0^2 \cos\,2\theta}{g} ~=~ 0
~\Rightarrow~ \cos\,2\theta ~=~ 0
~\Rightarrow~ 2\theta ~=~ \frac{\pi}{2}
~\Rightarrow~ \theta ~=~ \frac{\pi}{4} ~~\text{is the only critical point}
\end{gather*}
Use the Second Derivative Test to verify that $L$ has a local maximum at
$\theta = \pi/4$:
\begin{align*}
L''(\theta) ~=~ -\frac{4v_0^2 \sin\,2\theta}{g}
\quad\Rightarrow\quad L''(\pi/4) ~=~ -\frac{4v_0^2}{g} ~<~ 0 \quad\Rightarrow\quad \text{$L$ has a local maximum at $\theta = \frac{\pi}{4}$}
\end{align*}
...
ground $\bigg($with $L\left(\frac{\pi}{4}\right) = \frac{v_0^2}{g}$ being the maximum
horizontal distance$\bigg)$.\vspace{2mm}
...
when $\sin\,2\theta$ has its largest value (i.e. $1$), which occurs when
$\theta = \pi/4$.
\end{exmp}
...
First, we can rule out the possibility that $C$ falls outside of $A$
and $B$\footnote{i.e. $X(C) \notin [X(A),X(B)]$, where $X()$ is a
function that returns the X-coordinate of a point (and $X(A)<X(B)$).}
since, if it did, contrary to Fermat's Principle (and it's
consequences), the total distance $D$ would be larger than if $C$ were
directly beneath either $A$ or $B$. This ensures that $\theta_1$ and
$\theta_2$ are between $0$ and $\pi/2$ (inclusive), and that $0 \le x
\le L$.
...
\]
At the critical points of $D$, the equation $D'(x)=0$ holds:
\begin{align*}
D'(x) ~=~ \frac{x}{\sqrt{x^2 + y_1^2}} ~-~ \frac{L-x}{\sqrt{(L-x)^2 + y_2^2}} ~&=~ 0\\
\Rightarrow~\frac{x}{d_1} ~&=~ \frac{L-x}{d_2}\\
\Rightarrow~\cos\,\theta_1 ~&=~ \cos\,\theta_2\\
\Rightarrow~\theta_1 ~&=~ \theta_2 \quad\left(\because~\text{cosine is one-to-one over the interval $\ival{0}{\frac{\pi}{2}}$}\right)
\end{align*}
...
\begin{align*}
D'(x) ~=~ 0 \quad\Rightarrow\quad \frac{x}{\sqrt{x^2 + y_1^2}} ~-~ \frac{L-x}{\sqrt{(L-x)^2 + y_2^2}} ~&=~ 0\\
\Rightarrow\quad \frac{x}{\sqrt{x^2 + y_1^2}} ~&=~ \frac{L-x}{\sqrt{(L-x)^2 + y_2^2}}\\
\Rightarrow\quad \frac{x^2}{x^2 + y_1^2} ~&=~ \frac{(L-x)^2}{(L-x)^2 + y_2^2}\\
\Rightarrow\quad \cancel{(L-x)^2 x^2} ~+~ x^2 y_2^2 ~&=~ \cancel{(L-x)^2 x^2} ~+~ (L-x)^2 y_1^2\\
\Rightarrow\quad xy_2 ~&=~ (L-x)y_1 \quad\left(\because~\begin{minipage}{0.38\linewidth}Since all the quantities involved are guaranteed to be positive, $\sqrt{a^2}=\abs{a}=a$\end{minipage}\right)\\
\Rightarrow\quad x ~&=~ \frac{Ly_1}{y_1 + y_2} ~\text{is the only critical point,}~x \in \ival{0}{L}.
\end{align*}
Now compare the values of $D(x)$ at $x=0$, $x=L$, and
$x=\frac{Ly_1}{y_1 + y_2}$. Some tedius algebraic manipulation yields:
\begin{align*}
D(0) ~&=~ y_1 + \sqrt{L^2 ~+~ y_2^2} &\quad\Rightarrow\quad& &\left(D(0)\right)^2 ~&=~ L^2 ~+~ y_1^2 ~+~ y_2^2 ~+~ 2y_1\sqrt{L^2 + y_2^2}\\
D(L) ~&=~ \sqrt{L^2 + y_1^2} + y_2 &\quad\Rightarrow\quad& &\left(D(L)\right)^2 ~&=~ L^2 ~+~ y_1^2 ~+~ y_2^2 ~+~ 2y_2\sqrt{L^2 + y_1^2}\\
D\left(\tfrac{Ly_1}{y_1 + y_2}\right) ~&=~ \sqrt{L^2 ~+~ (y_1 + y_2)^2} &\quad\Rightarrow\quad& &\left(D\left(\tfrac{Ly_1}{y_1 + y_2}\right)\right)^2 ~&=~ L^2 ~+~ y_1^2 ~+~ y_2^2 ~+~ 2y_1y_2
\end{align*}
Since $y_2 < \sqrt{L^2 + y_2^2}$ and $y_1 < \sqrt{L^2 + y_1^2}$, then
$\left(D\left(\tfrac{Ly_1}{y_1 + y_2}\right)\right)^2$ is the smallest of the three values
above, so that $\left(D(x)\right)^2$ has its minimum value at $x=\frac{Ly_1}{y_1 + y_2}$,
...
\parpic[r]{\begin{tikzpicture}[scale=1.7,every node/.style={font=\small}]
 ...
 \draw [|{latex}-{latex}|] (0.5,-0.2) -- (1.5,-0.2) node[above,midway,inner sep=1pt] {$x$};
 \draw [latex-{latex}|] (1.5,-0.2) -- (2.7,-0.2) node[above,midway,inner sep=1pt] {$10-x$};
 \draw [|{latex}-{latex}|] (0.5,-0.33) -- (2.7,-0.33) node[fill=white,midway,inner sep=0pt] {$10$};
 \node [above,scale=1.19,xscale=-1] at (0.5,0.9) {\PHship};
...
\end{tikzpicture}}
 ...
 $T$ on the \emph{closed} interval $\ival{0}{10}$ will occur either at $x=\frac{16}{3}$, or at $x=0$, or $x=10$. Since
\[
...
\end{tikzpicture}}
 ...
 for $D^2$ must occur either at the endpoint $x=0$ or at $x=1/2$. But $D^2(0)=1 >
...
\newpage
%Begin Section 4.2
\section{Curve Sketching}\label{sec:curve-sketching}
...
and left as an exercise. Assume that $f''(x)>0$ on $(a,b)$, and let
$l(x)$ be the line joining $(a,f(a))$ and $(b,f(b))$, as in the
drawing on the right (which also suggests that $f(x)$ < $l(x)$ over
$(a,b)$, but can't be accepted as proof).

The goal is to show that $\left(g(x) = f(x) - l(x)\right) < 0$ on
$(a,b)$, since this will show that $f(x) < l(x)$ on $(a,b)$. Since $f$
and $l$ are both continuous on $\ival{a}{b}$ then so is $g$. As such,
the \CRef{thm:evt}{Extreme Value Theorem} guarantees that $g$ attains
a maximum (and minimum) value somewhere in $\ival{a}{b}$. Suppose the
(global) maximum of $g(x)$ occurs at an interior point $x=c$, i.e. for
some $c$ in the open interval $(a,b)$. Then $g'(c)=0$ and
$g''(c)=f''(c)-l''(c)=f''(c)>0$ (since $l(x)$ is a line and hence has
a second derivative of $0$ for all $x$). Then by the
\CRef{thm:2ndderivtest}{Second Derivative Test}, $g$ has a (local)
minimum at $x=c$, which contradicts $g$ having a (global) maximum at
$x=c$. Thus, the (global) maximum of $g$ cannot occur at an interior
point, so it must occur at one of the end points $x=a$ or $x=b$. In
other words, either $g(x)<g(a)$ or $g(x)<g(b)$ for all $x$ in
$(a,b)$. But $f(a)=l(a)$ and $f(b)=l(b)$, so $g(a)=0=g(b)$. Hence,
$g(x)<0$ for all $x$ in $(a,b)$, i.e. $f(x) < l(x)$ for all $x$ in
$(a,b).  \quad\checkmark$\vspace{2mm}\\ \divider
\vspace{3mm}
...
case the stray constant $a_0$ can be multiplied with $D$---amounting
to a vertical scaling, $kf(x)$, of a function $f(x)$ by a scalar
$k$---since that will not
...
\newpage
%Begin Section 4.3
\section{Numerical Approximation of Roots of Functions}\label{sec:numeric-root-approx}
...
Section~\ref{sec:continuity}. In fact, in Example \ref{exmp:ivt} the solution to the equation
...
The algorithm terminates whenever the desired accuracy is reached or
when $f'(x_n)=0$ for some $n \ge 0$, at which point you could start
over with a different initial guess $x_0$.}
...
\ref{sec:numeric-root-approx} could come in handy, since obtaining closed-form solutions might be
...
explanation was provided in Section~\ref{sec:derivative-limit-approach} for why positive or negative derivatives
