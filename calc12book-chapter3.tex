...
\begin{align*}
 ...
 0 < \abs{x - a} < \delta_1 \qquad\text{and}\quad 0 < \abs{x - a} < \delta_2 &\left(\because \delta = \min(\delta_1, \delta_2)\right)\\
 &\Rightarrow\quad \abs{f(x) ~-~ L_1} < \epsilon/2 \quad\text{and}\quad
 \abs{g(x) ~-~ L_2} < \epsilon/2 &\left(\text{as deduced previously}\right)\\
