\chapter{Topics in Differential Calculus}\label{ch:topics-in-differential-calculus}
%Begin Section 3.1
\section{Tangent Lines}\label{sec:tangent-lines}
...
in calculus. Recall from Chapter~\ref{ch:derivative} that by the Microstraightness Property a
...
was shown in Section~\ref{sec:derivative-inifinitesimal-approach} (namely, $\sin\,\dx = \dx$, so that $\sin\,x \approx x$
for $x \ll 1$).
\end{exmp}
\divider
\newpage
...
\begin{displaymath}
 ...
 \underbrace{\lim_{w \to x_0} ~\frac{f(w) ~-~ f(x_0)}{w ~-~ x_0} ~=~ f'(x_0)}_\text{from equation~\ref{eqn:wxderivative}} ~=~
 \text{slope of } T_P
\end{displaymath}
...
\newpage
 \item \emph{Smooth curves have tangent lines, nonsmooth curves do not.}\\
  ...
  $(0,0)$, because as shown in Example~\ref{exmp:absnondiff}, the derivative of $f(x)$ does not exist
...
\par\noindent Many lines---some of which are outlined with dashes---go
through the point of nonsmoothness in the figures above, but none of
them can be \emph{the} tangent line. Sharp edges and cusps have to be
``smoothed out'' to have a tangent line.
\end{itemize}
\divider
\vspace{3mm}
...
function of $x$. Let $\phi = \phi(x)$ be the \emph{smallest} angle that the tangent
line, $L$, to a curve $y = f(x)$ at $(x,f(x))$ makes with the positive $x$-axis, so that
...
\begin{figure}[ht]
 ...
 \end{tikzpicture}
 \caption[]{\\The values of the smallest angle, $\phi(x)$, between the positive $x$-axis and the tangent line, $L$, to the curve $y=f(x)$ at various $x$}
...
run in a right triangle, as shown in the figure on the right. The figure also
shows that, by definition of the tangent of an angle, $\tan\,\phi(x)$ also
...
\ref{fig:normalline} shows a natural way to define how a line $N$ can be
perpendicular to a \emph{curve} $y=f(x)$ at a point $P$ on the curve: if the line
...
\statedefn{defn:limit}{
 ...
 \end{displaymath}
 \[ \text{i.e.}\quad \lim_{x \to a} ~f(x) ~=~ L \quad\Leftrightarrow\quad 0 < \abs{x - a} < \delta ~\Rightarrow~ \abs{f(x)-L} ~<~ \epsilon \]
}
...
For example, the rules for limits---listed at Page~\pageref{thm:limitrules}---can be proved by
...
$\displaystyle\lim_{x \to a} g(x) = L_2$. (The goal now is to show that
$\displaystyle\lim_{x \to a} ~(f(x) + g(x)) = L_1 + L_2$.) Let $\epsilon > 0$.
Then $\epsilon/2 > 0$ (which we will use since the
\CRef{defn:limit}{definition} allows \emph{any} $\epsilon > 0$). By
\CRef{defn:limit}{definition} there exist numbers $\delta_1 > 0$ and
$\delta_2 > 0$ such that
\begin{align*}
 \lim_{x \to a} f(x) = L_1 \quad\Leftrightarrow\quad 0 < \abs{x - a} < \delta_1 ~&\Rightarrow~ \abs{f(x) ~-~ L_1} < \epsilon/2~\text{, and}\\
 \lim_{x \to a} g(x) = L_2 \quad\Leftrightarrow\quad 0 < \abs{x - a} < \delta_2 ~&\Rightarrow~ \abs{g(x) ~-~ L_2} < \epsilon/2 ~.
\end{align*}
\indent Now let $\delta = \min(\delta_1, \delta_2)$. Then $\delta > 0$ and
\begin{align*}
 0 < \abs{x - a} < \delta \quad&\Rightarrow\quad
 0 < \abs{x - a} < \delta_1 \qquad\text{and}\quad 0 < \abs{x - a} < \delta_2 \quad\left(\because \delta = \min(\delta_1, \delta_2)\right)\\
 &\Rightarrow\quad \abs{f(x) ~-~ L_1} < \epsilon/2 \quad\text{and}\quad
 \abs{g(x) ~-~ L_2} < \epsilon/2 \quad\left(\text{as deduced previously}\right)\\
 &\Rightarrow\quad \left(\abs{f(x) ~-~ L_1} ~+~ \abs{g(x) ~-~ L_2}\right) ~<~ \left(\epsilon/2 ~+~ \epsilon/2\right)\\
 &\Rightarrow\quad \underbrace{\abs{(f(x) ~-~ L_1) ~+~ (g(x) ~-~ L_2)}
   ~\le~ \left(\abs{f(x) ~-~ L_1} ~+~ \abs{g(x) ~-~
     L_2}\right)}_\text{Since $\abs{A+B} \le \abs{A} + \abs{B}$ for
   all real numbers $A$ and $B$} ~<~ \epsilon\\
 &\Rightarrow\quad \underbrace{\abs{f(x) ~+~ g(x) ~-~ (L_1 +
     L_2)}}_\text{Basic algebraic manipulation} ~\le~ \left(\abs{f(x)
   ~-~ L_1} ~+~ \abs{g(x) ~-~ L_2}\right) ~<~ \epsilon
\end{align*}
\indent Thus, by \CRef{defn:limit}{definition},
\[
 0 < \abs{x-a} < \delta ~\Rightarrow~
 \abs{f(x) ~+~ g(x) ~-~ (L_1 + L_2)} ~<~ \epsilon
 \quad\Leftrightarrow\quad \lim_{x \to a} ~(f(x) + g(x)) = L_1 + L_2
 \quad\checkmark
\]
\end{exmp}
\divider
...
functions---in many cases a simple analysis of the function, often from its graph, is all that is
needed.
...
$f(x)$, i.e. $f(a)$ does not have to be defined. So it will \emph{not} always
...
depending on how $x$ approached $1$? In that case the limit would not have existed. The
...
\statedefn{defn:onesidedlimit}{
 ...
 $a$, written as\vspace{-1em}
 \begin{displaymath}
  \lim_{x \to a+} ~f(x) ~=~ L ~,\vspace{-1em}
 \end{displaymath}\vspace{-1em}
 if $f(x)$ approaches $L$ as $x$ approaches $a$ for values of $x$ larger
 than $a$.
 \[ \text{i.e.}\quad \lim_{x \to a+} ~f(x) ~=~ L \quad\Leftrightarrow\quad 0 < x - a < \delta ~\Rightarrow~ \abs{f(x) ~-~ L} ~<~ \epsilon \]
 \quad\\
 ...
 than $a$.
 \[ \text{i.e.}\quad \lim_{x \to a-} ~f(x) ~=~ L \quad\Leftrightarrow\quad 0 < a - x < \delta ~\Rightarrow~ \abs{f(x) ~-~ L} ~<~ \epsilon \]
 \end{center}
}

\noindent The following is an immediate consequence of the foregoing
...
be made larger than any positive number by picking $x$ sufficiently close to $a$:
 ...
 \end{equation*}
\[ \text{i.e.}\quad \lim_{x \to a} ~f(x) ~=~ \infty \quad\Leftrightarrow\quad 0 < \abs{x - a} < \delta ~\Rightarrow~ f(x) ~>~ M \]
}
...
approaches $0$ from the right, $y=1/x$\\approaches $\infty$, that is,
$\displaystyle \lim_{x \to 0+}~\dfrac{1}{x} ~=~ \infty$.\\
As $x$ approaches $0$ from the left, $y=1/x$ approaches $-\infty$,\\
that is, $\displaystyle \lim_{x \to 0-}~\dfrac{1}{x} ~=~ -\infty$.\\
Since the right limit and the left limit are not equal,
...
approaches $0$ from either the right\\ or the left, $y=1/x^2$ approaches
$\infty$, that is,
\[
...
For a real number $L$, the limit of a function $f(x)$ equals $L$
 ...
 \end{equation*}
 \[ \text{i.e.}\quad \lim_{x \to \infty} ~f(x) ~=~ L \quad\Leftrightarrow\quad x > N ~\Rightarrow~ \abs{f(x) - L} < \epsilon \]
}
...
The above definitions can be modified accordingly for $L$ replaced by
either $\infty$ or $-\infty$.

One way to interpret the statement
...
\noindent For example, obviously $2x^3 = O(x^3)$, by picking $M = 2$ and
any positive number as $x_0$. In general, $f(x) = O(g(x))$ means that $f$ exhibits the
same long-term behavior as $g$, up to a constant multiple. You can think of $g$
as the more basic (or standard) ``type'' of function that describes $f$, insofar as the long-term
...
\begin{exmp}\label{exmp:bigOexmp}
  ...
  \begin{align*}
    &\underbrace{
      \abs{5x^4 - 2} ~\le~ \Abs{5x^4} ~+~ \abs{-2}
    }_\text{$\because~ \abs{a + b} \le \abs{a} + \abs{b}$ for all $a, b \in \Reals$}
    ~=~ 5\,\Abs{x^4} ~+~ 2 &\text{for all $x \in \Reals$}\\
    &\Abs{x^4} = x^4 \ge 1 &\text{for all $x \ge 1$}\\
    \Rightarrow~ &\abs{5x^4 - 2} ~\le~ \underbrace{
      5\,\Abs{x^4} ~+~ 2 ~\le~ 5\,\Abs{x^4} ~+~ 2\,\Abs{x^4}
    }_{\because~ 1 \le \Abs{x^4} \text{ for all } x \ge 1}
    ~=~ 7\,\Abs{x^4} &\text{for all $x \ge 1$}\\
    \Rightarrow~ &\abs{5x^4 - 2} ~\le~ 7\,\Abs{x^4} &\text{for all $x \ge 1$}\\
    \Leftrightarrow~ &5x^4 - 2 = O(x^4) &\text{By \CRef{defn:bigO}{definition}, with $M=7$ and $x_0=1$.}
  \end{align*}
\end{exmp}
\divider
\newpage
...
\noindent Evaluate $\displaystyle\lim_{x \to \infty}~\left(\sqrt{x + 1} \,-\, \sqrt{x}\right)~$.
...
$\infty \left( \text{i.e.} \displaystyle \lim_{x \to \infty} 1/x = 0\right)$.
\vspace{-0.625em}
\end{exmp}
\divider
...
Real Analysis}, New York: Springer-Verlag, 1977. or \S\,3.7
``L'H\^{o}pital's Rule and Indeterminate Forms'' in \textsc{Feldman,
  J., A. Rechnitzer and E. Yeager}, \emph{CLP-2 Integral Calculus},
\url{https://personal.math.ubc.ca/~CLP/CLP2/}, 2017. or
\textsc{Jipsen, P.}, \emph{Proof of L'H\^{o}pital's Rule},
\url{https://math.chapman.edu/~jipsen/mathposters/L'Hospital's%20Rule.pdf},
  2019.}; a simplified form is stated below:
...
\text{then,}\quad&\lim_{x \to a}~\frac{f(x)}{g(x)} ~=~ \lim_{x \to a}~\frac{f'(x)}{g'(x)} ~.
\end{align*}
\vspace{-0.5em}
This also holds for $x \to \pm\infty$ besides $x \to a, a \in \Reals$.}
}
...
=~ &~0 \quad(\text{since the numerator is $2$ and $e^x \to \infty$ as $x \to \infty$.})
\end{align*}
...
\]
\[
\Rightarrow\quad \lim_{x \to \infty}~\frac{\sin\,x}{x} ~=~ 0 ~\text{by
  the Squeeze Theorem, since $\displaystyle\lim_{x \to
    \infty}~-\frac{1}{x} ~=~ 0 ~=~ \displaystyle\lim_{x \to
    \infty}~\frac{1}{x}$.  }
\]
\end{exmp}
\divider
...
composition of continuous functions) is also continuous. In addition,
the following holds true:

\statethm{thm:contlimit}{If $f$ is a continuous function and
$\displaystyle\lim_{x \to a}~g(x) = L \in \Reals$, i.e. the limit exists and is finite, then:
\begin{equation}\label{eqn:lim-cont-comp}
\lim_{x \to a}~f(g(x)) ~=~ f\left(\lim_{x \to a}~g(x)\right) ~=~ f(L)
\end{equation}
The same relation also holds for one-sided limits.
}

\noindent This seemingly trivial result is useful in evaluating the
indeterminate forms $0^0$, $\infty^0$, and $1^{\infty}$. The
trick---somewhat parallaling the technique of logarithmic
differentiation (and more clearly illustrated in Example
\ref{exmp:zerotozerolim})---is to start by taking the natural
logarithm of the limit of such an indeterminate form and then work
backwards through Equation~\ref{eqn:lim-cont-comp}. The natural
logarithm function, $\ln\,x$, will be the continuous function $f(x)$;
and the indeterminate limit, presumed to exist, will be $\displaystyle
\lim_{x \to a}~g(x)$. This $\displaystyle \ln\left(\lim_{x \to
  a}~g(x)\right)$ form, working backwards through
Equation~\ref{eqn:lim-cont-comp}, is equal to the form $\displaystyle
\lim_{x \to a}~\ln(g(x))$, which can be determined after logarithmic
properties are used to further simplify $\ln(g(x))$. The original
indeterminate limit, $\left(\lim_{x \to a}~g(x)\right)$ is then
recovered by evaluating $e^{\ln\left(\lim_{x \to
    a}~g(x)\right)}$. \index{indeterminate form}
...
&=~ \lim_{x \to 0+}~\ln\,x^x \quad\left[\text{working backwards through Equation~\ref{eqn:lim-cont-comp}:}~\displaystyle f\left(\lim_{x \to a}~g(x)\right) ~=~ \lim_{x \to a}~f(g(x))\right]\\[4pt]
&=~ \lim_{x \to 0+}~x\;\ln\,x \quad\left[\text{using logarithmic property to simplify the indeterminate form}\right]\quad\to\quad 0 \cdot (-\infty)\\
&=~ \lim_{x \to 0+}~\frac{\ln\,x}{1/x} \quad\to\quad\frac{-\infty}{\infty}\\[6pt]
&=~ \lim_{x \to 0+}~\frac{1/x}{-1/x^2} \quad\left(\text{by L'H\^{o}pital's Rule}\right) ~=~ \lim_{x \to 0+}~(-x) ~=~ 0
\end{align*}
...
\[
\lim_{x \to a}~(f(x) - f(a)) ~=~ \lim_{x \to a}~\left((f(x) - f(a)) \cdot \dfrac{x - a}{x - a}\right) ~=~
...
shown in the picture on the right---but---recall from
Example~\ref{exmp:absnondiff} in
Section~\ref{sec:derivative-limit-approach} that---it is not
...
interval $(c,d)$\footnote{The real numbers can't be listed in a
sequence and as such there is no ``next'' number to $c$ nor a ``previous''
number to $d$ that could be dubbed the extreme values in the interval
$(c,d)$ (see Footnote~\ref{footnote:dense-reals} in
Chapter~\ref{ch:derivative}).}. The Intermediate Value Theorem says
...
continuous for all $x \in \Reals$, in particular it is continuous on
$x \in \ival{0}{1}$. So since $\left(f(1) = -0.459698\right) < 0 <
\left(f(0) = 1\right)$, then by the \CRef{thm:ivt}{Intermediate Value
  Theorem} there is a number $c \in (0,1)$ such that $f(c) = 0$ since
$0 \in \big(f(1), f(0)\big)$. This means that $\cos\,c ~-~ c ~=~ 0
~\Rightarrow~ \cos\,c ~=~ c$. That is, a solution to the equation
$\cos\,x = x$ exists and is a number $x = c \in (0,1)$.\vspace{-2mm}
\end{exmp}
\divider
\vspace{2mm}
...
\newpage
%Begin Section 3.4
\section{Implicit Differentiation}\label{sec:implicit-differentiation}
...
you can use the \CRef{thm:chainrule}{Chain Rule} to solve for $\dydx$. The example below illustrates
...
3x^2 ~+~ 3 ~&=~ 2y \cdot \dydx \quad\left(\text{by the Chain Rule}\right)\\[5pt]
\text{So,}\quad \dydx ~&=~ \frac{3x^2 ~+~ 3}{2y}
...
1 ~+~ \dydx ~&=~ 3x^2 ~+~ 3y^2 \cdot \dydx \quad\left(\text{by the Chain Rule}\right)\\[6pt]
\text{So,}\quad\dydx ~&=~ \frac{3x^2 ~-~ 1}{1 ~-~ 3y^2}
...
2x ~+~ 2y \cdot \dydx ~=~ 0 \quad\left(\text{by the Chain Rule}\right)\\[4pt]
...
\newpage
%Begin Section 3.5
\section{Related Rates}\label{sec:related-rates}
...
The known rates of change in that relation are then used to determine an
unknown related rate of change.\index{related rates}
...
\]
\[
\Rightarrow\quad \frac{d\!h}{\dt} ~=~ \frac{1}{30000} \dVdt ~=~ \frac{1}{30000}\cdot 60000
~=~ 2~\text{ft/min} .
\]
\end{exmp}
...
$\theta = \pi/6$.\footnote{This is denoted $\dxdt\Biggr|_{\theta=\pi/6}$: the vertical bar
means ``evaluated at'' the value of the subscript to the right of the bar.}
From the adjacent figure,
\begin{align*}
&x ~=~ 100\;\cot\,\theta \quad\left(\because~ \tan\,\theta ~=~ \frac{100}{x}\right)\\
\Rightarrow\quad &\dxdt ~=~ -100\;\csc^2\theta \cdot
\frac{d\negmedspace\theta}{\dt} ~=~ -100\;\csc^2\theta \cdot (-0.05)
~=~ 5\;\csc^2\theta \quad\left(\because~
\frac{d\negmedspace\theta}{\dt} = -0.05~\text{rad/min}\right)\\
\Rightarrow\quad &\dxdt\Biggr|_{\theta=\pi/6} ~=~ 5\;\csc^2(\pi/6) ~=~ 5\;(2)^2 ~=~ 20~\text{ft/min}
\end{align*}
\end{exmp}
\divider
\newpage
...
volume of the cylinder, respectively. Since
$\frac{\dr}{\dt} = -3$ cm/min and $\frac{d\negmedspace h}{\dt} = 2$ cm/min,
differentiating $V = \pi\,r^2 \,h$ with respect to $t$ on both sides yields:
\vspace{-0.7em}
\begin{gather*}
  \ddt V ~=~ \ddt (\pi\,r^2 \,h) ~=~ \pi \ddt (r^2 \,h) ~=~ \pi
  \underbrace{\left(\ddt (r^2) \cdot h + r^2 \cdot
    \ddt(h)\right)}_\text{Product Rule} ~=~ \pi \underbrace{\left(
    2\,r\,\drdt\cdot\,h + \,r^2 \cdot\frac{d\negmedspace h}{\dt}
    \right)}_\text{Chain Rule} ~=~ 2\pi \,r\,\drdt\,h +
  \pi\,r^2\,\frac{d\negmedspace h}{\dt} \\
  \Rightarrow\quad
...
Note that this is identical to Equation (\ref{eqn:dffprimedx}) in Section~\ref{sec:derivative-inifinitesimal-approach}.
...
\begin{align*}
  ...
  \Rightarrow\quad\cancel{\dx}\,\frac{d(fg)}{\cancel{\dx}} ~&=~
  \cancel{\dx}\,\left(f\,\frac{\dg}{\cancel{\dx}} ~+~
...
\begin{align*}
 \intertext{The differential version of the ideal gas law in Equation (\ref{eqn:gaslaw}) can now be proved:}
 &PV& ~&=~ RT& &\left(\text{Ideal Gas Law}\right)\\
\Rightarrow\quad &d(PV)& ~&=~ d(RT)& &\left(\text{Taking the differential of both sides of the equation}\right)\\
\Rightarrow\quad &d(PV)& ~&=~ R \cdot \dT& &\left(\text{By the Constant Multiple Rule}\right)\\
\Rightarrow\quad &V \; \dP ~+~ P \; \dV& ~&=~ \frac{PV}{T} \; \dT& &\left(\text{By the Product Rule and since
 $R = \frac{PV}{T}$}\right)\\
\Rightarrow\quad &\frac{V\;\dP}{PV} ~+~ \frac{P\;\dV}{PV}& ~&=~ \frac{\dT}{T}& &\left(\text{After dividing both
 sides by $PV$}\right)\\
\Rightarrow\quad &\frac{\dP}{P} ~+~ \frac{\dV}{V}& ~&=~ \frac{\dT}{T}& &\checkmark
\end{align*}
...
=~ (M - \dm)(v + \dv)
\intertext{and the momentum of the fuel that was burnt and expelled out the rear, which is}
(v - v_E)\,\dm.
\end{gather*}
\vspace{-4em}
\begin{align*}
\intertext{So, by conservation of momentum,}
Mv ~&=~ (M - \dm)(v + \dv) ~+~ (v - v_E)\,\dm\\
\Rightarrow\quad \cancel{Mv} ~&=~ \cancel{Mv} ~-~ \cancel{v\,\dm} ~+~ M\,\dv ~-~ (\dm)(\dv) ~+~ \cancel{v\,\dm} ~-~ v_E\,\dm\\
\Rightarrow\quad v_E\,\dm ~&=~ M\,\dv ~-~ (\dm)(\dv)\\
\Rightarrow\quad v_E\,\dm ~&=~ M\,\dv
~\bigg(\because~\underbrace{(\dm)(\dv) =
  \left(m'(t)\,\dt\right)\left(v'(t)\,\dt\right)}_\text{By
  Equation~\ref{eqn:dffprimedx}} ~=~ m'(t)v'(t)(\dt)^2 ~=~ m'(t)v'(t)
\cdot 0 = 0\bigg)
\quad\checkmark
\end{align*}
Furthermore,
\[
v_E\,\dm ~=~ M\,\dv \quad\Rightarrow\quad \frac{v_E\,\dm}{\dt} ~=~ \frac{M\,\dv}{\dt} \quad\Rightarrow\quad M\,\dot{v} ~=~ \dot{m}\,v_E
\]
using the dot notation---mentioned in Section~\ref{sec:derivative-inifinitesimal-approach}---for the derivative with
...
Letting $f$ be the natural logarithm function, $\ln$, and letting $g = u$ in the
differential version of the Chain Rule yields the following useful result:

\statethm{thm:dlnu}{\begin{center}
$d(\ln\,u) ~=~ \ddu\left(\ln\,u\right)\du ~=~ \dfrac{1}{u}\du ~=~ \dfrac{\du}{u}$
\end{center}}
This is often used in a differential version of the technique of logarithmic
differentiation\index{logarithmic differentiation} discussed in Section~\ref{sec:exponential-and-natural-logarithm-functions}.
...
\begin{tikzpicture}[every node/.style={font=\small}]
 ...
 \draw[densely dashed,thick] (0,1.5) -- (0,2) node[midway,right] {$\dr$};
 \draw[line width=1pt,-latex] (2.0,1.3) -- (5,1.3)
  node[midway,above,align=left] {slice and roll flat};
 \node at (1.75,0) {$d\negmedspace A$};
 \fill (0,0) circle (2.5pt);
 \begin{scope}[>=latex,shift={(5,1)}]
  \filldraw[fill=fillcolor,line width=1pt] (0,0) -- (6,0) -- (5.5,0.5) -- (0.5,0.5) --cycle;
  \draw[|<->|] (0.5,0.8) -- (5.5,0.8) node[fill=white,midway] {$2\pi r$};
  \draw[|<->|] (0,-0.3) -- (6,-0.3) node[fill=white,midway] {$2\pi(r+\dr)=2\pi r+2\pi\dr$};
  \draw[|-|] (0,-0.50) -- (0.5,-0.50) node[midway,below] {$\pi\dr$};
  \draw[-] (0.5,-0.50) -- (5.5,-0.50) node[midway,below] {$2\pi r$};
  \draw[|-|] (5.5,-0.50) -- (6,-0.50) node[midway,below] {$\pi\dr$};
  \draw[dotted,thick] (0.5,0) -- (0.5,0.5) node[midway,right] {$\dr$};
 ...
 \end{scope}
 \begin{scope}[shift={(12.0,-1.5)},yscale=1,xscale=-1]
   \filldraw[fill=fillcolor,line width=1pt] (0,0) -- (1.5,0) node[midway,below] {$\pi\;\dr$}
   -- (1.5,1.5) node[midway,left] {$\dr$} -- cycle;
   \draw (1.3,0) -- (1.3,0.2) -- (1.5,0.2);
 \end{scope}
\end{tikzpicture}
\end{center}

Slice that ring along the dashed line then roll it flat\footnote{If
you try to do this with paper and scissors and can't roll the ring
flat, proceed by taking one piece of thread around the inner circle
and another piece around the outer one.}, yielding a trapezoid
...
example, recall Example \ref{exmp:relrate3} in Section~\ref{sec:related-rates} on related rates,
